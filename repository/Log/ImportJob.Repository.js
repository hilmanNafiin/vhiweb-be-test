...
class ImportJob {
  async getOneRepository(params) {
    try {
      const response = await _.from("import_jobs").where(params).first();
  ...
  async createRepository(params) {
    try {
      const result = await _.insert(params).into("import_jobs");
  ...
  async updateRepository(params, condition) {
    try {
      const result = await _.from("import_jobs")
        .where(condition)
        .update(params);
