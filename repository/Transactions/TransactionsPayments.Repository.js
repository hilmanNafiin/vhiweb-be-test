...
class TransactionsPayments {
  async getOneRepository(params) {
    try {
      const response = await _.from("transactionspayments")
        .where(params)
        .first();
      console.log(response, params, "ransactionspayments");
  ...
  async createRepository(params) {
    try {
      const result = await _.insert(params).into("transactionspayments");
  ...
  async updateRepository(params, condition) {
    try {
      const result = await _.from("transactionspayments")
        .where(condition)
        .update(params);
...
module.exports = new TransactionsPayments();
